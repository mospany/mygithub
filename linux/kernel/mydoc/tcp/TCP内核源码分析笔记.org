...
*** 调用时机
**** init
	 拥塞算法的初始化函数，进行特定的初始化过程。当传输控制块的某种拥塞控制算法被选中时调用。
	 调用时机：
     tcp_cong.c:96:	if (icsk->icsk_ca_ops->init)
     tcp_cong.c:97:		icsk->icsk_ca_ops->init(sk);
     tcp_cong.c:269:		if (sk->sk_state != TCP_CLOSE && icsk->icsk_ca_ops->init)
     tcp_cong.c:270:			icsk->icsk_ca_ops->init(sk);
...
**** release
	 当关闭接口，或传输控制块选择了另一个拥塞控制算法时，原先拥塞控制算法的此接口在正式设置前被调用，进行清理工作。当关闭套接口时，
	 此接口也被调用。如果不需要进行清理工作也可以不实现此接口。
	 调用时机：
...
**** ssthresh
	 ...
	 调用时机：
	 - tcp_enter_cwr()
     cp_input.c:823:			tp->snd_ssthresh = icsk->icsk_ca_ops->ssthresh(sk);
	 #+BEGIN_SRC c -n	 
	 ...
	 - tcp_enter_frto()
     cp_input.c:2059:			tp->snd_ssthresh = icsk->icsk_ca_ops->ssthresh(sk);
     cp_input.c:2062:			tp->snd_ssthresh = icsk->icsk_ca_ops->ssthresh(sk);
	 #+BEGIN_SRC c -n	 	 
	 ...
	 - tcp_enter_loss()
     cp_input.c:2202:		tp->snd_ssthresh = icsk->icsk_ca_ops->ssthresh(sk);
	 #+BEGIN_SRC c -n	
	 ...
	 - tcp_fastretrans_aletr()
     cp_input.c:3051:			tp->snd_ssthresh = icsk->icsk_ca_ops->ssthresh(sk);
	 #+BEGIN_SRC c -n		 
...
**** min_cwnd
	 计算并返回拥塞窗口最小值。
	 调用时机: tcp_fastretrans_alert() --> tcp_cwnd_down() --> tcp_cwnd_min().
	 - tcp_cwnd_min()
	 #+BEGIN_SRC c -n		 
...
**** cong_avoid
	 在拥塞避免模式下重新计算拥塞窗口。
	 - 调用时机: 
     tcp_input.c:3658:			tcp_cong_avoid(sk, ack, prior_in_flight);
     tcp_input.c:3663:			tcp_cong_avoid(sk, ack, prior_in_flight);
	 #+BEGIN_SRC c -n		 	 
...
**** set_state
	 在拥塞状态改变时，此接口会被调用。
	 - 调用时机:
     tcp.c:2062:	tcp_set_ca_state(sk, TCP_CA_Open);
     tcp_input.c:831:		tcp_set_ca_state(sk, TCP_CA_CWR);
     tcp_input.c:2099:	tcp_set_ca_state(sk, TCP_CA_Disorder);
     tcp_input.c:2164:	tcp_set_ca_state(sk, TCP_CA_Loss);
     tcp_input.c:2243:	tcp_set_ca_state(sk, TCP_CA_Loss);
     tcp_input.c:2702:	tcp_set_ca_state(sk, TCP_CA_Open);
     tcp_input.c:2771:			tcp_set_ca_state(sk, TCP_CA_Open);
     tcp_input.c:2794:		tcp_set_ca_state(sk, state);
     tcp_input.c:2891:		tcp_set_ca_state(sk, TCP_CA_Loss);
     tcp_input.c:2960:				tcp_set_ca_state(sk, TCP_CA_Open);
     tcp_input.c:2971:				tcp_set_ca_state(sk, TCP_CA_Open);
     tcp_input.c:3057:		tcp_set_ca_state(sk, TCP_CA_Recovery);
     tcp_minisocks.c:429:		tcp_set_ca_state(newsk, TCP_CA_Open);
...
**** cwnd_event
	 用于通知拥塞控制算法内部的事件接口，当有拥塞控制的事件发生时被调用。
	 - 调用时机:	 
     tcp_input.c:2071:		tcp_ca_event(sk, CA_EVENT_FRTO);
     tcp_input.c:2203:		tcp_ca_event(sk, CA_EVENT_LOSS);
     tcp_input.c:2782:	tcp_ca_event(sk, CA_EVENT_COMPLETE_CWR);
     tcp_input.c:3615:		tcp_ca_event(sk, CA_EVENT_FAST_ACK);
     tcp_input.c:3632:		tcp_ca_event(sk, CA_EVENT_SLOW_ACK);
     tcp_output.c:134:	tcp_ca_event(sk, CA_EVENT_CWND_RESTART);
     tcp_output.c:655:		tcp_ca_event(sk, CA_EVENT_TX_START);
	 #+BEGIN_SRC c -n		 	 	 
...
**** undo_cwnd	 
	 在撤销“缩小拥塞窗口”时，　如果实现此接口，则会调用此接口撤销拥塞窗口。
	 - 调用时机:
    tcp_input.c:2653:		if (icsk->icsk_ca_ops->undo_cwnd)
    tcp_input.c:2654:			tp->snd_cwnd = icsk->icsk_ca_ops->undo_cwnd(sk);
	 #+BEGIN_SRC c -n		 	 	 	   
...
**** pkts_acked
	 当发送方接收到ACK后，　有段被确认时，此接口被调用。参数num_acked为此次ACK确认的段数。
	 - 调用时机:
     tcp_input.c:3296:		if (ca_ops->pkts_acked) {
     tcp_input.c:3311:			ca_ops->pkts_acked(sk, pkts_acked, rtt_us);
	 - tcp_clean_rtx_queue()	   
	 #+BEGIN_SRC c -n		 	 	 	   	 
...
**** get_info
	 提供给inet_diag的获取信息接口。
	 调用时机:
       inet_diag.c:168:	    icsk->icsk_ca_ops && icsk->icsk_ca_ops->get_info)
       inet_diag.c:169:		icsk->icsk_ca_ops->get_info(sk, ext, skb);	 
	 - inet_csk_diag_fill()
	 #+BEGIN_SRC c -n		 	 	 	   	 	 
